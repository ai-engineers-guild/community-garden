...
def build_weekly_llm_pack(events: list[CommunityEvent], metrics: dict, graph_metrics: dict, period: str, max_messages: int = 1200) -> str:
    ...
    lines.append("")
    lines.append("This pack is generated for external LLM analysis. It preserves chronological flow, reply context, metrics, graph hints, and evidence ids.")
    lines.append("")
    ...
    for day in sorted({e.timestamp.date().isoformat() for e in selected}):
        ...
        for e in [x for x in selected if x.timestamp.date().isoformat() == day]:
            ...
            if not text and e.event_type.value != "admin_action":
                continue
            reply = f" reply_to={e.relation.reply_to_event_id}" if e.relation.reply_to_event_id else ""
            lines.append(f"- [{e.timestamp.strftime('%H:%M')}] `{e.event_id}` {_name(e)}{reply}: {text[:1200]}")
    lines.append("")
    lines.append("## Analysis instruction")
    lines.append("Use this pack to produce evidence-based community intelligence. Do not invent facts. Every strong claim must point to event ids or metric blocks.")
    return "\n".join(lines).strip() + "\n"
