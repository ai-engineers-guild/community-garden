...
def calculate_basic_metrics(
    events: list[CommunityEvent], member_count_snapshot: int | None = None
) -> dict:
    messages = [
        e for e in events if e.event_type in {EventType.MESSAGE_CREATED, EventType.MESSAGE_EDITED}
    ]
    ...
    replies = [
        e for e in messages if e.relation.reply_to_event_id or e.relation.reply_to_message_id
    ]
    ...
    top5_share = (
        sum(c for _, c in by_actor.most_common(5)) / total_messages if total_messages else 0
    )
    active_speaker_ratio = (
        len(unique_actors) / member_count_snapshot if member_count_snapshot else None
    )
    one_shot_authors = [a for a, c in by_actor.items() if c == 1]
    returning_authors = [a for a, c in by_actor.items() if c >= 2]

    # Entropy for author diversity
    author_probs = [c / total_messages for c in by_actor.values()] if total_messages else []
    author_diversity_entropy = -sum(p * log2(p) for p in author_probs if p > 0)

    # ERR and Answered messages
    answered_messages = len(
        set(e.relation.reply_to_event_id for e in replies if e.relation.reply_to_event_id)
    )
    answered_message_rate = answered_messages / total_messages if total_messages else 0
    err = len(replies) / total_messages if total_messages else 0
    self_sustained_threads = sum(
        1
        for target, reps in by_reply_to_raw.items()
        if reps and all(not (e.actor and e.actor.is_admin) for e in reps)
    )
    days = sorted(by_day)
    return {
        "activity": {
        ...
        "breadth": {
            ...
            "author_diversity_entropy": round(author_diversity_entropy, 4),
            "active_speaker_ratio": round(active_speaker_ratio, 4)
            if active_speaker_ratio is not None
            else None,
            "silent_reader_estimate": member_count_snapshot - len(unique_actors)
            if member_count_snapshot
            else None,
        },
        "response": {
            "reply_count": len(replies),
            "answered_message_rate": round(answered_message_rate, 4),
            "err_engagement_rate": round(err, 4),
            "question_candidates": len(questions),
            "unanswered_question_candidates": len(unanswered),
            "unanswered_question_rate": round(len(unanswered) / len(questions), 4)
            if questions
            else 0,
            "median_first_response_minutes": round(median(response_times), 2)
            if response_times
            else None,
        },
        "admin": {
            "admin_message_count": len(admin_messages),
            "admin_message_share": round(len(admin_messages) / total_messages, 4)
            if total_messages
            else 0,
        ...
        "readability": {
            ...
            "long_text_messages": sum(
                1 for e in text_messages if len(e.content.clean_text or "") > 800
            ),
