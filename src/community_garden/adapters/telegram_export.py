...
from community_garden.models import (
    Actor,
    CommunityEvent,
    Content,
    EventType,
...
from community_garden.utils import read_json
...
class TelegramExportAdapter(SourceAdapter):
    ...
    async def iter_events(self) -> AsyncIterator[CommunityEvent]:
        ...
        chat_id = str(
            self.chat_id
            or data.get("id")
            or data.get("personal_information", {}).get("user_id")
            or "telegram_export"
        )
    ...
    def _message_to_event(
        self, msg: dict[str, Any], chat_id: str, chat_title: str
    ) -> CommunityEvent | None:
        msg_id = msg.get("id")
        if msg_id is None:
            return None
